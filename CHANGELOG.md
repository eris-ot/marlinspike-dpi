...
- **Bundle features** `ot`, `it`, `l2`, and `all` aggregate the per-protocol leaf features for convenience (`all = ["ot", "it", "l2"]`). Select a family with `--features ot`, or compose leaves directly. ~80 leaf features cover OT/ICS, IT, and link-layer protocols.
- The four multi-protocol decoder files (`link_layer`, `it_app`, `it_basic`, `recognizers`) were split into per-protocol directory modules so each protocol is fully excluded from compilation — not merely unregistered — when its feature is off. Note: `cargo test` is run with default features (the test suite assumes all protocols are present); subset builds are validated via `cargo build`/`cargo check`.

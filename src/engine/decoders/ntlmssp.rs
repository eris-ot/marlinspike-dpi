...
inventory::submit!(crate::engine::decoders::DecoderRegistration {
    name: "ntlmssp",
    factory: || Box::new(NtlmsspDecoder),
});
...
fn sec_buf_slice(buf: &[u8], off: usize) -> Option<&[u8]> {
...
fn parse_av_pairs(blob: &[u8]) -> AvPairs {
    ...
    let mut pos = 0;
    while let Some(av_id) = u16le(blob, pos) {
        let av_len = match u16le(blob, pos + 2) {
            Some(v) => v as usize,
            None => break,
        };
        pos += 4;
        ...
        let val = match blob.get(pos..pos + av_len) {
            Some(b) => b,
            None => break,
        };
...
fn parse_type1(blob: &[u8], flags: u32, chunk: &StreamChunk<'_>, out: &mut Vec<BronzeEvent>) {
    let domain = sec_buf_slice(blob, 16)
        .map(|b| decode(b, flags))
        .unwrap_or_default();
    let workstation = sec_buf_slice(blob, 24)
        .map(|b| decode(b, flags))
        .unwrap_or_default();
    emit_tx(
        "ntlmssp_negotiate",
...
fn parse_type2(blob: &[u8], flags: u32, chunk: &StreamChunk<'_>, out: &mut Vec<BronzeEvent>) {
    let target_name = sec_buf_slice(blob, 12)
        .map(|b| decode(b, flags))
        .unwrap_or_default();
    ...
    let nb_computer = av
        .as_ref()
        .map(|a| a.nb_computer.clone())
        .unwrap_or_default();
    let nb_domain = av.as_ref().map(|a| a.nb_domain.clone()).unwrap_or_default();
    let dns_computer = av
        .as_ref()
        .map(|a| a.dns_computer.clone())
        .unwrap_or_default();
    let dns_domain = av
        .as_ref()
        .map(|a| a.dns_domain.clone())
        .unwrap_or_default();
    ...
    let hostname = if !nb_computer.is_empty() {
        nb_computer.clone()
    } else if !dns_computer.is_empty() {
        dns_computer
    } else {
        chunk.context.src_ip.to_string()
    };
    let ad_domain = if !nb_domain.is_empty() {
        nb_domain
    } else {
        dns_domain
    };
    ...
    out.push(new_event(
        ...
        BronzeEventFamily::AssetObservation(AssetObservation {
            ...
            model: None,
            firmware: None,
            hostnames: if hostname.is_empty() {
                vec![]
            } else {
                vec![hostname]
            },
...
fn parse_type3(blob: &[u8], flags: u32, chunk: &StreamChunk<'_>, out: &mut Vec<BronzeEvent>) {
    ...
    let domain = sec_buf_slice(blob, 28)
        .map(|b| decode(b, flags))
        .unwrap_or_default();
    let username = sec_buf_slice(blob, 36)
        .map(|b| decode(b, flags))
        .unwrap_or_default();
    let workstation = sec_buf_slice(blob, 44)
        .map(|b| decode(b, flags))
        .unwrap_or_default();
    ...
    if !username.is_empty() {
        ...
        out.push(new_event(
            ...
            BronzeEventFamily::AssetObservation(AssetObservation {
                ...
                model: None,
                firmware: None,
                hostnames: if workstation.is_empty() {
                    vec![]
                } else {
                    vec![workstation]
                },
...
fn process_chunk(chunk: &StreamChunk<'_>, out: &mut Vec<BronzeEvent>) {
    ...
    if on_http
        && payload.first().is_some_and(|b| b.is_ascii())
        && payload.windows(10).any(|w| w == b"Negotiate ")
    {
        emit_tx(
            "ntlmssp_in_http_negotiate",
            attrs(&[
    ...
    while from + NTLMSSP_MAGIC.len() <= payload.len() {
        ...
        let msg_type = match u32le(blob, 8) {
            Some(t) => t,
            None => break,
        };
        ...
        if sp == 80 || dp == 80 {
            emit_anomaly(
                "medium",
                "NTLMSSP over plaintext HTTP — credential exposure risk",
                blob,
                chunk,
                out,
            );
        ...
        match msg_type {
            ...
            other => {
                emit_tx(
                    &format!("ntlmssp_unknown_type_{other}"),
                    attrs(&[
                ...
                emit_anomaly(
                    "low",
                    &format!("Unknown NTLMSSP message type: {other}"),
                    blob,
                    chunk,
                    out,
                );
...
mod tests {
    ...
    fn test_type2_challenge_basic() {
        ...
        let mut evs = Vec::new();
        NtlmsspDecoder.on_stream_chunk(&chunk(&pkt, ctx(445, 12345)), &mut evs);
    ...
    fn test_type2_challenge_avpairs() {
        ...
        assert_eq!(
            asset.identifiers.get("ad_domain").map(String::as_str),
            Some("EXAMPLE")
        );
    }
    ...
    fn test_type3_authenticate_asset() {
        ...
        pkt.extend_from_slice(&1u32.to_le_bytes()); // flags UNICODE
        pkt.extend_from_slice(&lm);
        pkt.extend_from_slice(&nt);
        pkt.extend_from_slice(&dom);
        pkt.extend_from_slice(&usr);
        pkt.extend_from_slice(&ws);

        let mut evs = Vec::new();
        NtlmsspDecoder.on_stream_chunk(&chunk(&pkt, ctx(12345, 445)), &mut evs);
        ...
        assert_eq!(
            asset.identifiers.get("ad_username").map(String::as_str),
            Some("alice")
        );
        assert_eq!(
            asset.identifiers.get("ad_domain").map(String::as_str),
    ...
    fn test_plaintext_http_anomaly() {
        ...
        let mut evs = Vec::new();
        NtlmsspDecoder.on_stream_chunk(&chunk(&pkt, ctx(12345, 80)), &mut evs);
    ...
    fn test_http_negotiate_header() {
        ...
        let mut evs = Vec::new();
        NtlmsspDecoder.on_stream_chunk(&chunk(pkt, ctx(80, 12345)), &mut evs);

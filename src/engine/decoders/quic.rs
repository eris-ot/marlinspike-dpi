...
fn version_label(v: u32) -> &'static str {
    match v {
        ...
        QUIC_V1                           => "quic_v1",
        QUIC_V2                           => "quic_v2",
        d if d >= DRAFT_MIN && d <= DRAFT_MAX => "quic_draft",
...
fn parse_long_header(buf: &[u8]) -> Option<LongHeader> {
    ...
    if dcid_len > 20 || 6 + dcid_len >= buf.len() { return None; }
    let dcid    = buf[6..6 + dcid_len].to_vec();
    let si      = 6 + dcid_len;             // index of scid_len byte
    let scid_len = buf[si] as usize;
    if scid_len > 20 || si + 1 + scid_len > buf.len() { return None; }
    let scid     = buf[si + 1..si + 1 + scid_len].to_vec();
    Some(LongHeader { first_byte, version, dcid, scid, consumed: si + 1 + scid_len })
}
...
fn read_varint(buf: &[u8], off: usize) -> Option<(u64, usize)> {
    ...
    match first >> 6 {
        ...
        2 => {
            ...
            Some((u32::from_be_bytes([first & 0x3f, buf[off+1], buf[off+2], buf[off+3]]) as u64, 4))
        }
        _ => {
            if off + 8 > buf.len() { return None; }
            Some((u64::from_be_bytes([first & 0x3f, buf[off+1], buf[off+2], buf[off+3],
                                      buf[off+4],   buf[off+5], buf[off+6], buf[off+7]]), 8))
        }
    }
}
...
impl SessionDecoder for QuicDecoder {
    ...
    fn on_datagram(&mut self, chunk: &StreamChunk<'_>, out: &mut Vec<BronzeEvent>) {
        ...
        if buf.len() < 6 {
            out.push(parse_anomaly_event(
                chunk.capture_id.to_string(), mk_env(), self.name(), "low",
                "quic long header truncated", buf,
            ));
            return;
        }
        ...
        if fixed_bit == 0 && version != VERSION_NEGOTIATION {
            out.push(parse_anomaly_event(
                chunk.capture_id.to_string(), mk_env(), self.name(), "low",
                "quic fixed bit is 0 in non-version-negotiation long-header packet", &buf[..1],
            ));
        }
        ...
        if dcid_len_raw > 20 {
            out.push(parse_anomaly_event(
                chunk.capture_id.to_string(), mk_env(), self.name(), "low",
                &format!("quic dcid length {} exceeds rfc 9000 maximum of 20", dcid_len_raw),
                buf,
            ));
            return;
        }
        ...
        let Some(lh) = parse_long_header(buf) else {
            out.push(parse_anomaly_event(
                chunk.capture_id.to_string(), mk_env(), self.name(), "low",
                "quic long header parse failed (truncated)", buf,
            ));
            return;
        };
        ...
        attrs.insert("quic_version".to_string(), version_label(lh.version).to_string());
        ...
        if lh.version == VERSION_NEGOTIATION {
            ...
            let mut svs  = Vec::new();
            while pos + 4 <= buf.len() {
                svs.push(format!("{:08x}", u32::from_be_bytes(
        ...
        out.push(new_event(
            ...
            BronzeEventFamily::ProtocolTransaction(ProtocolTransaction {
                ...
                request_summary: Some(format!(
                    "QUIC {} version={} dcid={}", operation, version_hex, dcid_hex
                )),
...
mod tests {
    ...
    fn ctx() -> PacketContext {
        PacketContext {
            ...
            src_port: 12345, dst_port: 443, vlan_id: None,
    ...
    fn test_handshake_v1() {
        ...
        assert!(!evs.iter().any(|e| matches!(e.family, BronzeEventFamily::AssetObservation(_))),
    ...
    fn test_version_negotiation_supported_versions() {
        ...
        pkt.push(4u8); pkt.extend_from_slice(&[0x01u8; 4]);      // dcid
        pkt.push(1u8); pkt.push(0xffu8);                         // scid
    ...
    fn test_dcid_length_too_large_anomaly() {
        ...
        assert!(anomaly.reason.contains("dcid") || anomaly.reason.contains("20"),
    ...
    fn test_fixed_bit_clear_long_header_anomaly() {
        ...
        pkt.extend_from_slice(&QUIC_V1.to_be_bytes());
        pkt.push(4u8); pkt.extend_from_slice(&[0x01u8; 4]);
        pkt.push(4u8); pkt.extend_from_slice(&[0x02u8; 4]);
        ...
        assert_eq!(anomaly.severity, "low");
        assert!(anomaly.reason.contains("fixed bit"), "reason: {}", anomaly.reason);
    }

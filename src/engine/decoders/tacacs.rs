...
impl TacacsHeader {
    ...
    fn body_unencrypted(&self) -> bool {
        self.flags & 0x01 != 0
    }
    fn version_byte(&self) -> u8 {
        (self.major_ver << 4) | self.minor_ver
    }
}
...
impl AuthenStartBody {
    fn parse(body: &[u8]) -> Option<Self> {
        ...
        if body.len() < 8 + user_len + port_len + rem_len {
            return None;
        }
        let mut off = 8;
        let username = String::from_utf8_lossy(&body[off..off + user_len]).into_owned();
        off += user_len;
        let port = String::from_utf8_lossy(&body[off..off + port_len]).into_owned();
        off += port_len;
...
fn tacacs_operation(pkt_type: u8, seq_no: u8) -> String {
    match pkt_type {
        ...
        TYPE_AUTHOR => {
            if seq_no == 1 {
                "tacacs_author_request".to_string()
            } else {
                "tacacs_author_reply".to_string()
            }
        }
        TYPE_ACCT => {
            if seq_no == 1 {
                "tacacs_acct_request".to_string()
            } else {
                "tacacs_acct_reply".to_string()
            }
        }
...
fn action_name(v: u8) -> &'static str {
    match v {
        ACTION_LOGIN => "LOGIN",
        ACTION_CHPASS => "CHPASS",
        ACTION_SENDPASS => "SENDPASS",
        _ => "UNKNOWN",
    }
}
...
fn authen_type_name(v: u8) -> &'static str {
    match v {
        AUTHEN_TYPE_ASCII => "ASCII",
        AUTHEN_TYPE_PAP => "PAP",
        AUTHEN_TYPE_CHAP => "CHAP",
        AUTHEN_TYPE_MSCHAP => "MS-CHAP",
        AUTHEN_TYPE_MSCHAPV2 => "MS-CHAPv2",
        _ => "UNKNOWN",
    }
}
    ...
    fn on_stream_chunk(&mut self, chunk: &StreamChunk<'_>, out: &mut Vec<BronzeEvent>) {
        ...
        if hdr.major_ver != MAJOR_VER_EXPECTED {
            out.push(parse_anomaly_event(
                ...
                "medium",
                &format!(
                    "unexpected tacacs+ major version 0x{:x}, expected 0xc",
                    hdr.major_ver
                ),
                payload,
            ));
        ...
        attrs.insert(
            "body_unencrypted".to_string(),
            if body_unencrypted { "true" } else { "false" }.to_string(),
        );
        ...
        if hdr.pkt_type == TYPE_AUTHEN && hdr.seq_no == 1 && body_unencrypted {
            ...
            if let Some(s) = AuthenStartBody::parse(body) {
                ...
                attrs.insert(
                    "authen_type_name".to_string(),
                    authen_type_name(s.authen_type).to_string(),
                );
                attrs.insert(
...
mod tests {
    ...
    fn test_authen_start_obfuscated() {
        ...
        assert!(
            assets
                .iter()
                .any(|a| a.role.as_deref() == Some("tacacs_server") && a.asset_key == "10.0.0.2")
        );
        assert!(
            assets
                .iter()
                .any(|a| a.role.as_deref() == Some("tacacs_client") && a.asset_key == "10.0.0.1")
        );
    }

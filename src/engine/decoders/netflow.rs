...
//!     decoded data flow record and one per template definition received.
//!   - `AssetObservation`     — for the exporter (src IP) and collector (dst IP),
//!     deduplicated per session; also once per unique source IP extracted from
//!     decoded flow records (role `"flow_endpoint"`).
    ...
    fn on_datagram(&mut self, chunk: &StreamChunk<'_>, out: &mut Vec<BronzeEvent>) {
        ...
        if data.len() < 2 {
            out.push(anomaly(
                chunk,
                "medium",
                "datagram too short for version field",
            ));
            return;
        }
...
impl NetFlowDecoder {
    ...
    fn handle_v9(&mut self, chunk: &StreamChunk<'_>, out: &mut Vec<BronzeEvent>) {
        ...
        while off + 4 <= data.len() {
            ...
            match fs_id {
                0 => {
                    self.parse_v9_template_flowset(fs_data, &exporter, source_id, false, chunk, out)
                }
                1 => {
                    self.parse_v9_template_flowset(fs_data, &exporter, source_id, true, chunk, out)
                }
                id if id >= 256 => {
    ...
    fn handle_ipfix(&mut self, chunk: &StreamChunk<'_>, out: &mut Vec<BronzeEvent>) {
        ...
        while off + 4 <= data.len() {
            ...
            match set_id {
                2 => self.parse_ipfix_template_set(
                    set_data,
                    &exporter,
                ...
                3 => self.parse_ipfix_template_set(
                    set_data,
                    &exporter,
                    observation_domain_id,
                    true,
    ...
    fn parse_v9_template_flowset(
        ...
        while off + 4 <= data.len() {
            ...
            if is_options {
                ...
                self.emit_template_lifecycle(
                    chunk,
                    "netflow_template_received",
                    template_id,
                    0,
                    true,
                    out,
    ...
    fn store_template(&mut self, key: TemplateKey, record: TemplateRecord) {
        if let std::collections::hash_map::Entry::Occupied(mut entry) =
            self.templates.entry(key.clone())
        {
        ...
        if self.templates.len() >= TEMPLATE_CAP
            && let Some(oldest) = self.template_order.pop_front()
        {
        ...
        let record =
            match self.templates.get(&key) {
                ...
                _ => {
                    ...
                    out.push(tx_event(
                        chunk,
                        "netflow_data_flowset_skipped".into(),
                        attr,
                        vec![],
                    ));
                    return;
...
fn format_ipv6(b: &[u8]) -> String {
    ...
    let (best_start, best_len) = {
        ...
        for (i, group) in groups.iter().enumerate().take(8) {
            if *group == 0 {
                if cl == 0 {
                    cs = i;
                }
    ...
    if best_len >= 2 {
        let left: Vec<String> = groups[..best_start]
            .iter()
            .map(|g| format!("{g:x}"))
            .collect();
...
mod tests {
    ...
    fn v9_template_then_data(
        ...
        // Record data
        b[off..off + 4].copy_from_slice(&src_ip);
        off += 4;
        b[off..off + 4].copy_from_slice(&dst_ip);
        off += 4;
        b[off..off + 2].copy_from_slice(&src_port.to_be_bytes());
        off += 2;
    ...
    fn has_anomaly(out: &[BronzeEvent]) -> bool {
        out.iter()
            .any(|e| matches!(&e.family, BronzeEventFamily::ParseAnomaly(_)))
    }

    fn anomaly_reason(out: &[BronzeEvent]) -> String {
        out.iter()
            .find_map(|e| match &e.family {
    ...
    fn v9_two_flowsets() {
        ...
        let tx = find_tx_by_op(&out, "netflow_v9_export").expect("netflow_v9_export missing");
        let a = attrs(tx);
    ...
    fn two_distinct_exporters() {
        ...
        assert_eq!(
            exporter_obs.len(),
            2,
            "expected 2 distinct exporter observations"
        );

        let ips: Vec<&str> = exporter_obs
            .iter()
            .map(|e| match &e.family {
    ...
    fn v9_data_flowset_decoded() {
        ...
        let dgram =
            v9_template_then_data(1, 257, [192, 168, 1, 10], [10, 20, 30, 40], 4567, 443, 99);
        dec.on_datagram(&chunk(&c, &dgram), &mut out);

        let flow = find_tx_by_op(&out, "netflow_flow_record").expect("netflow_flow_record missing");
        let a = attrs(flow);
    ...
    fn v9_data_flowset_no_template_anomaly() {
        ...
        assert!(
            has_anomaly(&out),
            "expected template_unresolved ParseAnomaly"
        );
    ...
    fn ipfix_enterprise_field_skipped() {
        ...
        b[off + 2..off + 4].copy_from_slice(&data_set_len.to_be_bytes());
        off += 4;
        b[off..off + 4].copy_from_slice(&[1, 2, 3, 4]);
        off += 4;
        ...
        assert!(
            !has_anomaly(&out),
            "should not emit anomaly for enterprise fields"
        );
        ...
        let flow = find_tx_by_op(&out, "ipfix_flow_record").expect("ipfix_flow_record missing");
        let a = attrs(flow);
        assert_eq!(a["sourceIPv4Address"], "1.2.3.4");
        // Enterprise field stored with pen suffix
        assert!(
            a.keys().any(|k| k.contains("pen_12345")),
            "enterprise field not stored: {a:?}"
        );
    }
    ...
    fn ipfix_data_set_flow_record() {
        ...
        let dgram = ipfix_template_then_data(99, 258, [172, 31, 0, 1], [8, 8, 8, 8], 54321, 53);
        dec.on_datagram(&chunk(&c, &dgram), &mut out);
    ...
    fn lru_eviction_at_cap() {
        ...
        assert!(
            !dec.templates.contains_key(&evicted_key),
            "oldest template not evicted"
        );
    }
    ...
    fn idle_flush_clears_state() {
        ...
        dec.on_datagram(&chunk(&c, &dgram), &mut out);
        assert!(
            !dec.templates.is_empty(),
            "templates should be non-empty after datagram"
        );
        assert!(
            !dec.seen.flow_endpoints.is_empty(),
            "flow_endpoints should be non-empty"
        );
        ...
        assert!(
            dec.templates.is_empty(),
            "templates not cleared after idle_flush"
        );
        assert!(
            dec.template_order.is_empty(),
            "template_order not cleared after idle_flush"
        );
        assert!(
            dec.seen.flow_endpoints.is_empty(),
            "flow_endpoints not cleared after idle_flush"
        );
    }

...
impl SessionDecoder for AdsDecoder {
    ...
    fn on_idle_flush(&mut self, _timestamp: DateTime<Utc>, out: &mut Vec<BronzeEvent>) {
        for (_, p) in self.pending.drain() {
            out.push(unpaired_tx(p.capture_id, p.envelope, p.operation, p.attributes, "request_only"));
        }
    }
}
...
impl AdsDecoder {
    fn drain(&mut self, chunk: &StreamChunk<'_>, out: &mut Vec<BronzeEvent>) {
        loop {
            if self.buf.len() < ADS_MIN_FRAME { break; }

            let reserved   = u16::from_le_bytes([self.buf[0], self.buf[1]]);
            let ams_len    = u32::from_le_bytes([self.buf[2], self.buf[3], self.buf[4], self.buf[5]]) as usize;
    ...
    fn process(&mut self, frame: &[u8], reserved: u16, chunk: &StreamChunk<'_>, out: &mut Vec<BronzeEvent>) {
        let env = build_envelope(
            &chunk.context, chunk.interface_id, chunk.frame_index, chunk.timestamp,
            chunk.segment_hash, TransportProtocol::Tcp, Some("ads"),
            chunk.captured_len, chunk.session_key.clone(),
        );

        if reserved != 0 {
            out.push(parse_anomaly_event(
                chunk.capture_id.to_string(), env.clone(), self.name(),
                "medium", "AMS/TCP reserved field non-zero", frame,
            ));
        }
        ...
        if h.len() < AMS_PKT_HDR {
            out.push(parse_anomaly_event(
                chunk.capture_id.to_string(), env, self.name(),
                "medium", "AMS packet header truncated", frame,
            ));
            return;
        }
        ...
        if data_len > ADS_MAX_DATA_LEN {
            out.push(parse_anomaly_event(
                chunk.capture_id.to_string(), env, self.name(),
                "high", "AMS data_len exceeds 16 MiB — likely malformed", frame,
            ));
            return;
        }
        ...
        if !matches!(cmd_id, 1..=9) {
            out.push(parse_anomaly_event(
                chunk.capture_id.to_string(), env.clone(), self.name(),
                "low", &format!("unknown ADS command ID 0x{cmd_id:04x}"), frame,
            ));
        }
        ...
        if state_flags & FLAG_RESPONSE != 0 {
            ...
            if let Some(p) = self.pending.remove(&req_key) {
                out.push(unpaired_tx(p.capture_id, p.envelope, p.operation, p.attributes, &status));
            } else {
                out.push(unpaired_tx(
                    chunk.capture_id.to_string(), env, operation, attrs, "response_only",
                ));
            }
        } else {
            ...
            if self.pending.len() >= ADS_PENDING_CAP {
                ...
                if let Some(k) = self.pending.keys().next().cloned() { self.pending.remove(&k); }
            }
...
mod tests {
    ...
    fn frame(
        src_n: [u8;6], src_p: u16, dst_n: [u8;6], dst_p: u16,
        cmd: u16, flags: u16, err: u32, invoke: u32, reserved: u16,
    ) -> Vec<u8> {
        let mut f = Vec::new();
        f.extend_from_slice(&reserved.to_le_bytes());
        f.extend_from_slice(&(AMS_PKT_HDR as u32).to_le_bytes());
        f.extend_from_slice(&dst_n); f.extend_from_slice(&dst_p.to_le_bytes());
        f.extend_from_slice(&src_n); f.extend_from_slice(&src_p.to_le_bytes());
    ...
    fn request_alone_no_transaction() {
        ...
        let mut out = Vec::new();
        let f = frame([10,0,0,1,1,1], 1000, [10,0,0,2,1,1], 851, 2, 0x04, 0, 42, 0);
    ...
    fn request_response_pair_ok() {
        let mut dec = AdsDecoder::default();
        let mut out = Vec::new();
        let sn = [10,0,0,1,1,1]; let dn = [10,0,0,2,1,1];
    ...
    fn response_nonzero_error_code() {
        let mut dec = AdsDecoder::default();
        let mut out = Vec::new();
        let sn = [10,0,0,3,1,1]; let dn = [10,0,0,4,1,1];
        dec.on_stream_chunk(&chunk(&frame(sn,2000,dn,851, 4,0x04,0,    99,0), ctx(2000,48898), "s3"), &mut out);
        dec.on_stream_chunk(&chunk(&frame(dn,851,sn,2000, 4,0x05,0x710,99,0), ctx(48898,2000), "s3"), &mut out);
    ...
    fn reserved_nonzero_anomaly_medium() {
        let mut dec = AdsDecoder::default();
        let mut out = Vec::new();
        let f = frame([1,2,3,4,5,6], 3000, [6,5,4,3,2,1], 851, 2, 0x04, 0, 1, 1);
    ...
    fn unknown_command_id_anomaly_and_operation() {
        let mut dec = AdsDecoder::default();
        let mut out = Vec::new();
        let f = frame([10,1,1,1,1,1], 4000, [10,1,1,2,1,1], 851, 0xFF, 0x04, 0, 5, 0);
    ...
    fn two_netids_two_asset_observations() {
        let mut dec = AdsDecoder::default();
        let mut out = Vec::new();
        let fa = frame([10,0,0,1,1,1], 5001, [10,0,0,10,1,1], 851, 2, 0x04, 0, 1, 0);
        let fb = frame([172,16,0,5,1,1], 5002, [10,0,0,10,1,1], 851, 2, 0x04, 0, 2, 0);
        dec.on_stream_chunk(&chunk(&fa, ctx(5001, 48898), "s6"), &mut out);
        dec.on_stream_chunk(&chunk(&fb, ctx(5002, 48898), "s6"), &mut out);
        let obs: Vec<_> = out.iter().filter(|e| matches!(e.family, BronzeEventFamily::AssetObservation(_))).collect();
        assert_eq!(obs.len(), 2);
    }
    ...
    fn same_netid_one_observation() {
        ...
        let count = out.iter().filter(|e| matches!(e.family, BronzeEventFamily::AssetObservation(_))).count();
        assert_eq!(count, 1);
    }
    ...
    fn fragmented_reassembly() {
        let mut dec = AdsDecoder::default();
        let mut out = Vec::new();
        let f = frame([10,0,0,1,1,1], 7000, [10,0,0,2,1,1], 851, 3, 0x04, 0, 55, 0);
        let mid = f.len() / 2;

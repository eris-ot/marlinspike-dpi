...
impl SessionDecoder for SmtpDecoder {
    ...
    fn on_stream_chunk(&mut self, chunk: &StreamChunk<'_>, out: &mut Vec<BronzeEvent>) {
        ...
        if session.tls_active || session.done {
            return;
        }
        ...
        let text = match std::str::from_utf8(chunk.payload) {
            ...
            Err(_) => {
                out.push(parse_anomaly_event(
                    ...
                    "smtp",
                    "low",
                    "non-utf8 bytes on smtp session",
                    chunk.payload,
                ));
                return;
            }
...
fn on_server_line(
    ...
    let Ok(code) = line[..3].parse::<u16>() else {
        emit_anomaly(chunk, line, out);
        return;
    };
    ...
    if code == 220 {
        ...
        if let Some(ref v) = vendor {
            attrs.insert("server_software".to_string(), v.clone());
        }
...
fn on_client_line(
    ...
    let mut attrs: BTreeMap<String, String> =
        BTreeMap::from([("argument".to_string(), rest.to_string())]);

    let operation: &str = match verb.as_str() {
        "HELO" => {
            attrs.insert("domain".to_string(), rest.to_string());
            "smtp_helo"
        }
        "EHLO" => {
            attrs.insert("domain".to_string(), rest.to_string());
        ...
        "MAIL" => {
            if let Some(addr) = extract_angle_addr(rest) {
                attrs.insert("address".to_string(), addr);
            }
            "smtp_mail_from"
        }
        "RCPT" => {
        ...
        "STARTTLS" => {
            session.awaiting_starttls_220 = true;
            "smtp_starttls"
        }
        "AUTH" => {
            let mech = rest
                ...
                .to_string();
            if !mech.is_empty() {
                attrs.insert("auth_mechanism".to_string(), mech);
            }
            "smtp_auth"
        }
        _ => {
...
mod tests {
    ...
    fn test_banner_postfix() {
        ...
        let mut out = Vec::new();
        dec.on_stream_chunk(
            &chunk(b"220 mail.example.com ESMTP Postfix\r\n", ctx(25, 60000)),
            &mut out,
        );
        ...
        let banner = t
            .iter()
            .find(|t| t.operation == "smtp_banner")
            .expect("smtp_banner");
        ...
        assert!(
            banner
                .attributes
                .get("banner_text")
                .map(|s| s.contains("mail.example.com"))
                .unwrap_or(false)
        );
    }
    ...
    fn test_ehlo_domain() {
        ...
        dec.on_stream_chunk(
            &chunk(b"220 smtp.example.com ESMTP\r\n", ctx(25, 60000)),
            &mut Vec::new(),
        );

        let mut out = Vec::new();
        dec.on_stream_chunk(
            &chunk(b"EHLO client.example.com\r\n", ctx(60000, 25)),
            &mut out,
        );
        ...
        let ehlo = t
            .iter()
            .find(|t| t.operation == "smtp_ehlo")
            .expect("smtp_ehlo");
        assert_eq!(
            ehlo.attributes.get("domain").map(String::as_str),
    ...
    fn test_mail_from_address() {
        ...
        let mut out = Vec::new();
        dec.on_stream_chunk(
            &chunk(b"MAIL FROM:<alice@example.com>\r\n", ctx(60000, 25)),
            &mut out,
        );
        ...
        let tx = t
            .iter()
            .find(|t| t.operation == "smtp_mail_from")
            .expect("smtp_mail_from");
        assert_eq!(
            tx.attributes.get("address").map(String::as_str),
    ...
    fn test_rcpt_to_address() {
        ...
        let mut out = Vec::new();
        dec.on_stream_chunk(
            &chunk(b"RCPT TO:<bob@example.com>\r\n", ctx(60000, 25)),
            &mut out,
        );
        ...
        let tx = t
            .iter()
            .find(|t| t.operation == "smtp_rcpt_to")
            .expect("smtp_rcpt_to");
        assert_eq!(
            tx.attributes.get("address").map(String::as_str),
    ...
    fn test_starttls_then_opaque() {
        ...
        assert!(
            txs(&out1).iter().any(|t| t.operation == "smtp_starttls"),
            "smtp_starttls expected"
        );
        ...
        assert!(
            txs(&out2).is_empty(),
            "no transaction events after STARTTLS 220"
        );

        // Further client data must be silently dropped.
        let mut out3 = Vec::new();
        dec.on_stream_chunk(
            &chunk(b"TLS HANDSHAKE BYTES\r\n", ctx(60000, 25)),
            &mut out3,
        );
    ...
    fn test_smtps_port_465() {
        ...
        let mut out = Vec::new();
        dec.on_stream_chunk(
            &chunk(b"\x16\x03\x03\x00\x01\xff", ctx(60000, 465)),
            &mut out,
        );
        ...
        assert_eq!(
            t[0].attributes.get("note").map(String::as_str),
        ...
        assert!(
            o.iter()
                .any(|o| o.identifiers.get("port").map(String::as_str) == Some("465"))
        );
    ...
    fn test_garbage_line_anomaly() {
        ...
        dec.on_stream_chunk(
            &chunk(
                b"hello world this is garbage data longer\r\n",
                ctx(60000, 25),
            ),
            &mut out,
        );
        ...
        assert!(
            anomalies.iter().any(|a| a.severity == "low"),

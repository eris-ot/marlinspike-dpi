...
const VENDOR_IDS: &[(&[u8], &str)] = &[
    ...
    (&[0x1e, 0x2b, 0x51, 0x69, 0x9d, 0x3c, 0xe8, 0x2d], "MS-Vid-Initial-Contact"),
    ...
    (
        &[0x12, 0xf5, 0xf2, 0x8c, 0x45, 0x71, 0x68, 0xa9,
          0x70, 0x2d, 0x9f, 0xe2, 0x74, 0xcc, 0x02, 0xd4],
        "Cisco-Unity",
    ),
...
impl IkeHeader {
    fn major(&self) -> u8 { self.version_byte >> 4 }
...
fn walk_payloads(buf: &[u8], mut next_payload: u8, mut offset: usize) -> (Vec<String>, bool) {
    ...
    while next_payload != 0 {
        if offset + 4 > buf.len() { malformed = true; break; }
        let this_next   = buf[offset];
        let payload_len = u16::from_be_bytes([buf[offset + 2], buf[offset + 3]]) as usize;
        if payload_len < 4 || offset + payload_len > buf.len() { malformed = true; break; }
...
impl SessionDecoder for IkeDecoder {
    ...
    fn interest(&self) -> &'static [DecoderInterest] {
        &[DecoderInterest::UdpPort(500), DecoderInterest::UdpPort(4500)]
    }
    ...
    fn on_datagram(&mut self, chunk: &StreamChunk<'_>, out: &mut Vec<BronzeEvent>) {
        ...
        let ike_buf = if is_natt {
            ...
            if payload[0] != 0x00 { return; } // ESP — encrypted, skip silently
            if payload[..4] != [0x00, 0x00, 0x00, 0x00] { return; }
            &payload[4..]
        } else {
            payload
        };

        let anomaly_envelope = || build_envelope(
            &chunk.context, chunk.interface_id, chunk.frame_index,
            chunk.timestamp, chunk.segment_hash, TransportProtocol::Udp,
            Some("ike"), chunk.captured_len, chunk.session_key.clone(),
        );

        let Some(hdr) = IkeHeader::parse(ike_buf) else {
            out.push(parse_anomaly_event(
                chunk.capture_id.to_string(), anomaly_envelope(),
                self.name(), "low", "ike header too short", ike_buf,
            ));
            return;
        };
        ...
        let status = if major == 2 {
            if hdr.flags & 0x20 != 0 { "response" } else { "request" }
        } else {
            let resp_zero  = hdr.responder_spi == [0u8; 8];
            let init_valid = hdr.initiator_spi != [0u8; 8];
            if init_valid && resp_zero { "request" }
            else if !resp_zero { "response" }
            else { "observed" }
        };
        ...
        if malformed {
            out.push(parse_anomaly_event(
                ...
                self.name(), "low", "malformed ike payload chain", ike_buf,
            ));
        }
...
mod tests {
    ...
    fn hdr(
        init_spi: [u8; 8], resp_spi: [u8; 8],
        next_payload: u8, version_byte: u8, exchange_type: u8, flags: u8, message_id: u32,
    ) -> Vec<u8> {
        let mut b = Vec::with_capacity(28);
        b.extend_from_slice(&init_spi);
        b.extend_from_slice(&resp_spi);
        b.push(next_payload); b.push(version_byte);
        b.push(exchange_type); b.push(flags);
    ...
    fn test_natt_esp_silently_skipped() {
        ...
        let mut evs = Vec::new();
        dec.on_datagram(&chunk(&payload, ctx(12345, 4500)), &mut evs);
        assert!(evs.is_empty(), "ESP on 4500 must emit nothing, got {}", evs.len());
    }
    ...
    fn test_bad_version_byte_anomaly() {
        ...
        let anomaly = evs.iter().find_map(|e| {
        ...
        assert!(anomaly.reason.contains("0x30"), "reason: {}", anomaly.reason);
    }

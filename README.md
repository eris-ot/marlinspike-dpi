...
> Protocol coverage (parsing depth varies — see [parse-depth-matrix.md](docs/parse-depth-matrix.md) for depth tiers): Modbus / Modbus-UDP / DNP3 / DNP3-SAv5 / IEC 60870-5-104 / IEC 61850 (MMS + GOOSE + SV) / S7comm / PROFINET / BACnet / BACnet/SC / EtherNet/IP / EtherNet/IP Class 1 I/O / OPC UA / OPC UA PubSub / OPC Classic / HART-IP / OMRON FINS / EtherCAT / MRP / PRP / PCCC / Sparkplug B / IEEE C37.118 synchrophasor / Beckhoff ADS / GE SRTP / Triconex TriStation / MELSEC SLMP / Yokogawa Vnet/IP / OSIsoft PI / GVCP / CIP Safety / Allen-Bradley CSP / Modicon UMAS / POWERLINK / SERCOS III / PTPv2 / Foundation Fieldbus HSE / AVTP / Emerson ROC Plus / Diameter — plus IT: DNS / DHCP / HTTP / TLS / SNMP / SSH / FTP / NTP / MQTT / MQTT-SN / Syslog / RADIUS / RadSec / ICMP / IGMP / SMB2/3 / Kerberos / LDAP / RDP / mDNS / WS-Discovery / DCE-RPC / TFTP / IKE / VNC / WinRM / NetBIOS / TACACS+ / WireGuard / NetFlow + IPFIX / QUIC / SMTP / OpenVPN / SIP / RTP / RTCP / CoAP / AMQP 1.0 / NTLMSSP.

**44 OT/ICS protocols (34 with full PDU parsing, 5 shallow, 5 recognition-only) + 40+ IT/infrastructure protocols. 21 anomaly detection signatures. VQT (Value/Quality/Timestamp) extraction for process historians. 9 OT protocols with typed `ProtocolFields` enum variants. Zero C dependencies. 880+ tests.**
...
- **OT/ICS protocol depth.** 44 OT/ICS protocols — 34 parsed to application layer (Deep or Full), 5 Shallow (header + fingerprint with some field extraction), 5 Recognition-only (port/magic-byte classification; see [parse-depth-matrix.md](docs/parse-depth-matrix.md)). The 34 deep-parsed protocols: Modbus/TCP, Modbus/UDP, DNP3, DNP3-SAv5, IEC 60870-5-104, IEC 61850 MMS, IEC 61850 GOOSE, IEC 61850 SV, S7comm, PROFINET, BACnet/IP, EtherNet/IP, EtherNet/IP Class 1 I/O, OPC UA (binary), OPC UA PubSub, HART-IP, OMRON FINS, EtherCAT, MRP, PRP, PCCC, Sparkplug B, IEEE C37.118, Beckhoff ADS, GE SRTP, MELSEC SLMP, GVCP, Modicon UMAS, POWERLINK, SERCOS III, PTPv2, AVTP, Allen-Bradley CSP, Emerson ROC Plus. Not just port identification — full PDU parsing with function codes, register values, device identity, and Value/Quality/Timestamp extraction.
...
- **Compile down to just the protocols you run.** Every protocol sits behind its own Cargo feature, so `--no-default-features --features "modbus,dnp3,s7comm"` (or a bundle like `ot`) drops everything else — dissector, session decoder, and L2 detector — from compilation, not merely from the registry. A 4-protocol build is ~44% smaller than the all-protocols default (2.1 MB vs 3.9 MB), which matters for embedded/firewall deployments shipping a fixed protocol set with minimal binary size and attack surface. Default is `all`, so it stays strictly opt-in. See [Selecting protocols](#selecting-protocols-smaller-builds).
- **880+ tests.** Unit tests for every dissector and detector. Integration tests for full-pipeline capture processing. Python bindings test suite. Zero ignored, zero flaky.
